                  ...
                  <template v-for="(subGroup, subGroupName) in getSubGroupsForDisplay(groupName, group)" :key="subGroupName">
                    ...
                    <div v-if="subGroupName" class="subgroup-container">
                      <div class="subgroup-header">
                        <v-icon size="small" class="mr-2">mdi-view-grid</v-icon>
                      ...
                      <div class="subgroup-content pl-4 pr-4">
                        ...
                        >
                          ...
                          <div v-if="!subGroup || subGroup.length === 0" class="text-center pa-8 text-grey">
                            <v-icon size="48" color="grey-lighten-1">mdi-drag-horizontal</v-icon>
                            ...
                            >
                              ...
                              <v-list-item-subtitle>
                                <div class="d-flex align-center mt-1">
                                  <span v-if="shouldShowFieldKey(field)" class="text-caption">鍵值：{{ field.field_key }}</span>
                    ...
                    <v-card-text>
                      ...
                      <div v-if="cascadingLevels.length > 1" class="mb-6 border pa-4 rounded">
                        ...
                        <template v-if="cascadingLevels[0] && cascadingLevels[0].options && cascadingLevels[0].options.length > 0">
                          ...
                          >
                            <div class="d-flex align-center mb-3">
                              <v-icon size="small" class="mr-2" color="primary">mdi-menu-right</v-icon>
              ...
              >
                <v-list density="compact" class="w-100">
                  <template v-for="(subGroup, subIndex) in getSubGroupsForGroup(groupName)" :key="subGroup.name">
                    ...
                    >
                      <template #prepend>
                        <v-icon class="mr-2" size="small" color="grey">mdi-drag</v-icon>
                      </template>
                      <v-list-item-title>
                        <div class="d-flex align-center">
...
const groupedFieldsForDesign = computed(() => {
  ...
  Object.keys(groups).forEach(groupName => {
    // 排序子群組內的欄位
    Object.keys(groups[groupName].subGroups).forEach(subGroupName => {
...
function openFieldDialog (field, index) {
  editingFieldIndex.value = index !== undefined ? index : null
  ...
  if (field) {
    ...
    if (needsCascadingLevels.value && fieldData.field_config.levels) {
      cascadingLevels.value = Array.isArray(fieldData.field_config.levels)
        ? JSON.parse(JSON.stringify(fieldData.field_config.levels)).map((level, index) => ({
          ...
          is_required: level.is_required !== undefined ? level.is_required : false,
          is_visible: level.is_visible !== undefined ? level.is_visible : true,
    ...
    // 載入配置 JSON
    try {
      fieldConfigJson.value = JSON.stringify(fieldData.field_config, null, 2)
    } catch {
...
function openGroupOrderDialog () {
  ...
  // 添加不在順序列表中的群組
  groupsWithFields.forEach(groupName => {
    if (!groupOrder.value.includes(groupName)) {
      ordered.push(groupName)
    }
  })
...
function updateFieldOrdersByGroupOrder () {
  ...
  unprocessedGroups.forEach(groupName => {
    const groupFields = fields.value.filter(f => f.field_group === groupName)
    groupFields.sort((a, b) => (a.display_order || 0) - (b.display_order || 0))
    groupFields.forEach(field => {
      field.display_order = orderCounter++
...
function getSubGroupsForDisplay (groupName, groupData) {
  ...
  if (groupData.subGroups) {
    Object.keys(groupData.subGroups).forEach(subGroupName => {
      ...
      if (!subGroupsList || !subGroupsList.find(sg => sg.name === subGroupName)) {
        if (groupData.subGroups[subGroupName] && groupData.subGroups[subGroupName].length > 0) {
...
function shouldShowFieldKey (field) {
  // 多層選單隱藏鍵值
  return field.field_type !== 'cascading_select'
}
...
async function saveForm () {
  ...
  try {
    ...
    if (isEditMode.value) {
      ...
      const formConfig = { ...formData.form_config }
      formConfig.group_order = groupOrder.value
      // 保存子群組資料
      const subGroupsData = {}
      if (subGroups.value && subGroups.value.size > 0) {
        subGroups.value.forEach((subGroupsList, groupName) => {
          if (subGroupsList && subGroupsList.length > 0) {
      ...
      for (let idx = 0; idx < existingFields.length; idx++) {
        const existingField = existingFields[idx]
...
const previewUngroupedFields = computed(() => {
  ...
  Object.keys(ungrouped.subGroups || {}).forEach(subGroupName => {
...
function toggleGroup (groupName) {
  ...
  if (allGroupsExpanded.value) {
    ...
    Object.keys(groupedFieldsForDesign.value).forEach(name => {
      if (name !== groupName) {
...
function toggleAllGroups () {
  ...
  if (allGroupsExpanded.value) {
    ...
    Object.keys(groupedFieldsForDesign.value).forEach(groupName => {
      expandedGroups.value.add(groupName)
    })
  } else {
    // 摺疊所有群組
